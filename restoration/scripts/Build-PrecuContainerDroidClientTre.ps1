<#
.SYNOPSIS
Builds and verifies the canonical Pre-CU account-container and droid client TRE.

.DESCRIPTION
Stages the bounded client feature payload, creates the TRE twice to prove that
the selected TreeFileBuilder produces deterministic output, and verifies both
the archive manifest and extracted bytes with the TreeFileExtractor located
next to TreeFileBuilder. The requested output is replaced only after every
verification succeeds.

.PARAMETER CompiledSharedRoot
Root containing the compiled shared-game IFFs at their archive-relative paths.

.PARAMETER ServerDataRoot
Optional serverdata root containing string/en/precu_container_droid.stf. If the
dedicated STF is not present there, CompiledSharedRoot is checked instead.
...
function Remove-VerifiedTemporaryDirectory {
    ...
    if (-not $resolvedPath.StartsWith($basePrefix, [StringComparison]::OrdinalIgnoreCase) -or
        -not $leafName.StartsWith("precu-container-droid-tre-", [StringComparison]::Ordinal)) {
...
$expectedArchivePaths = [string[]]@(
    ...
    "object/draft_schematic/droid/shared_survey_droid_transmitter.iff",
    "object/draft_schematic/droid/shared_worker_droid_transmitter.iff",
    "object/intangible/container/shared_craft_component_container.iff",
    "object/intangible/container/shared_droid_container.iff",
    "object/intangible/container/shared_resource_container.iff",
    "object/intangible/container/shared_ship_part_container_1000.iff",
    "object/intangible/container/shared_ship_part_container_2000.iff",
    "object/intangible/container/shared_ship_part_container_3000.iff",
    "object/intangible/container/shared_ship_part_container_4000.iff",
    "object/intangible/container/shared_ship_part_container_5000.iff",
    "object/intangible/container/shared_vehicle_container.iff",
    "object/tangible/datapad/shared_character_datapad.iff",
    "object/tangible/mission/shared_mission_survey_droid.iff",
    "object/tangible/mission/shared_mission_worker_droid.iff",
    "string/en/precu_container_droid.stf"
)
...
$sourceByArchivePath = @{}
foreach ($archivePath in $expectedArchivePaths | Where-Object { $_ -ne "string/en/precu_container_droid.stf" }) {
    $sourcePath = Join-Path $compiledSharedRootPath ($archivePath.Replace("/", "\"))
    if (-not (Test-Path -LiteralPath $sourcePath -PathType Leaf)) {
        throw "Canonical TRE input is missing: '$sourcePath'."
    }
    if ((Get-Item -LiteralPath $sourcePath).Length -eq 0) {
        throw "Canonical TRE input is empty: '$sourcePath'."
    }
    $sourceByArchivePath[$archivePath] = (Resolve-Path -LiteralPath $sourcePath).ProviderPath
}

$stfArchivePath = "string/en/precu_container_droid.stf"
$stfCandidates = [System.Collections.Generic.List[string]]::new()
if ($serverDataRootPath) {
    $stfCandidates.Add((Join-Path $serverDataRootPath ($stfArchivePath.Replace("/", "\"))))
}
$stfCandidates.Add((Join-Path $compiledSharedRootPath ($stfArchivePath.Replace("/", "\"))))
$stfSourcePath = @(
    $stfCandidates | Where-Object { Test-Path -LiteralPath $_ -PathType Leaf }
) | Select-Object -First 1
if (-not $stfSourcePath) {
    throw "Dedicated client STF is missing. Checked: $($stfCandidates -join ', ')."
}
if ((Get-Item -LiteralPath $stfSourcePath).Length -eq 0) {
    throw "Dedicated client STF is empty: '$stfSourcePath'."
}
$sourceByArchivePath[$stfArchivePath] = (Resolve-Path -LiteralPath $stfSourcePath).ProviderPath

$temporaryBase = [IO.Path]::GetFullPath([IO.Path]::GetTempPath())
$temporaryRoot = Join-Path $temporaryBase (
    "precu-container-droid-tre-{0}-{1}" -f $PID, [Guid]::NewGuid().ToString("N")
)
...
try {
    ...
    $responseFileName = "__precu_container_droid.rsp"
    $firstBuildName = "__precu_container_droid.first.tre"
    $secondBuildName = "__precu_container_droid.second.tre"
    ...
    $listResult = Invoke-TreeTool `
        ...
        -WorkingDirectory $temporaryRoot `
        -LogStem "__precu_container_droid.list"
    ...
    $extractionDirectoryName = "__precu_container_droid.extracted"
    $extractResult = Invoke-TreeTool `
        -FilePath $treeFileExtractorPath `
        -ArgumentList @("-e", $firstBuildName, $extractionDirectoryName) `
        -WorkingDirectory $temporaryRoot `
        -LogStem "__precu_container_droid.extract"
    ...
    $pendingOutputPath = Join-Path $outputDirectory (
        ".{0}.build-{1}-{2}.tmp" -f $outputLeaf, $PID, [Guid]::NewGuid().ToString("N")
    )
    ...
    if ($finalHash -cne $firstBuildHash) {
        throw "Published archive hash changed while copying to '$outputFullPath'."
    }
    ...
    [pscustomobject]@{
        ...
        StfSource = $sourceByArchivePath[$stfArchivePath]
    }
}
finally {
