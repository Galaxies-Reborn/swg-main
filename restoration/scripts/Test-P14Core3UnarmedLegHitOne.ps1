...
if (-not $actions.Contains("public int unarmedLegHit1(") -or -not $actions.Contains('combatStandardAction("unarmedLegHit1"')) { throw "Action hook drifted." }
...
if ($d.Count -ne 1 -or $d[0].percentAddFromWeapon -cne "2.5" -or
    $d[0].animDefault -cne "attack_special_foot_stomp" -or $d[0].anim_unarmed -cne "attack_special_foot_stomp" -or
...
if ($o.Count -ne 1 -or $o[0].healthCostMultiplier -cne "1.75" -or
    ...
    $o[0].targetPool -cne "ACTION" -or $o[0].speedMultiplier -cne "2" -or
...
if ($p.Count -ne 1 -or $p[0].combatSpam -cne "grondastomp") { throw "Spam row drifted." }

$k = @($skills | Where-Object name -ceq "combat_unarmed_support_02")
if ($k.Count -ne 1 -or ([string]$k[0].COMMANDS).Split(",") -cnotcontains $name) { throw "Skill owner drifted." }

foreach ($token in @('UNARMED_LEG_ONE_COMMAND = "unarmedLegHit1"', 'ORIGINAL_UNARMED_LEG_ONE_COMMAND', 'unarmedLegOneCommand=', 'canPerformUnarmedLegOne=')) {
    if (-not $fixture.Contains($token)) { throw "Fixture drifted: $token" }
}

if ($Expectation -eq "Ready") {
    $contract = Get-Content -LiteralPath (Join-Path $restorationRoot "contracts/p14-core3-unarmed-leg-hit-one.json") -Raw | ConvertFrom-Json
    if ($contract.status -ne "ready" -or $contract.runtimeEvidence.result -ne "passed" -or
        ...
        $contract.runtimeEvidence.spamKey -cne "grondastomp_hit" -or
...
Write-Host "Publish 14.1 Core3 Unarmed Leg Hit I contract passed."
