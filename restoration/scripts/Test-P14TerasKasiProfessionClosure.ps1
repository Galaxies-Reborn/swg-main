...
$contract = Get-Content (Join-Path $root ([string]$manifest.contracts.p14TerasKasiProfessionClosure)) -Raw | ConvertFrom-Json
...
Write-Host "Publish 14.1 Teras Kasi profession closure checks:"
...
) "p14.teras-kasi-profession.complete-four-by-four-shape"
Assert-C (
...
if ($Expectation -ceq "Ready") {
    ...
    ) "p14.teras-kasi-profession.build.ready-and-hashed"
    Assert-C (
    ...
    ) "p14.teras-kasi-profession.client-publications"
    Assert-C (
        ...
        [string]$runtime.selectedProfession -ceq "combat_unarmed_novice" -and
    ...
    ) "p14.teras-kasi-profession.runtime-graph"
}
if ($failures.Count) { throw "Teras Kasi profession closure failed: " + ($failures -join ", ") }
Write-Host "Publish 14.1 Teras Kasi profession closure passed."
