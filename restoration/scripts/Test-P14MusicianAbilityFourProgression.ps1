...
$contract = Get-Content (
    Join-Path $root ([string]$manifest.contracts.
        p14MusicianAbilityFourProgression)
...
$e = $contract.publish14Evidence.abilityFour
...
Write-Host "Publish 14.1 Musician Ability IV checks:"
Assert-C (
    $box.Count -eq 1 -and
...
) "p14.musician-ability-four.skill.authentic"
Assert-C (
    $fixture.Contains("prepareMusicianAbilityFour") -and
    $fixture.Contains("purchaseMusicianAbilityFour") -and
    $fixture.Contains("observeSurrenderMusicianAbilityFour") -and
    $fixture.Contains("NGE_FEATURED_SOLO_ABILITY") -and
    $fixture.Contains("OMNI_BOX_SCHEMATIC")
) "p14.musician-ability-four.fixture-reversible"
...
if ($Expectation -ceq "Ready")
{
    ...
    Assert-C (
        ...
        [int]$b.clientNativeBuild.protocolVersion -eq 70
    ) "p14.musician-ability-four.build.ready-and-hashed"
    Assert-C (
        [bool]$live.purchase.passed -and
        [int]$live.purchase.musicXpBefore -eq 225000 -and
        [int]$live.purchase.pointCost -eq 2 -and
        -not [bool]$live.purchase.featuredSoloPresent -and
        [bool]$live.purchase.schematicPresent -and
        [string]$live.purchase.modifierDeltasAfter -ceq "25,25" -and
        [bool]$live.surrender.abilityThreeRetained -and
        [bool]$live.surrender.ventriloquismRetained -and
        [bool]$live.surrender.bandfillSchematicRetained -and
    ...
    ) "p14.musician-ability-four.live-purchase-surrender"
}
if ($failures.Count)
{
    throw "Musician Ability IV contract failed: " +
        ($failures -join ", ")
}
Write-Host ""
Write-Host "Publish 14.1 Musician Ability IV contract passed."
