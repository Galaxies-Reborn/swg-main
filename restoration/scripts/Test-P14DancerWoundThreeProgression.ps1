...
$contract = Get-Content (
    Join-Path $root ([string]$manifest.contracts.
        p14DancerWoundThreeProgression)
...
foreach ($path in @($skillsPath, $fixturePath))
{
    ...
    {
        throw "Required Dancer Wound III source is missing: $path"
    }
}
...
$wound = @($skills |
    Where-Object NAME -CEQ ([string]$e.woundThree.name))
...
Write-Host "Publish 14.1 Dancer Wound III checks:"
Assert-Contract (
    $wound.Count -eq 1 -and
    [string]$wound[0].PARENT -ceq [string]$e.woundThree.parent -and
    [string]$wound[0].GRAPH_TYPE -ceq
        [string]$e.woundThree.graphType -and
    [string]$wound[0].IS_TITLE -ceq
        [string]$e.woundThree.isTitle -and
    [string]$wound[0].IS_PROFESSION -ceq
        [string]$e.woundThree.isProfession -and
    [string]$wound[0].MONEY_REQUIRED -ceq
        [string]$e.woundThree.moneyRequired -and
    [string]$wound[0].POINTS_REQUIRED -ceq
        [string]$e.woundThree.pointsRequired -and
    [string]$wound[0].SKILLS_REQUIRED -ceq
        [string]$e.woundThree.skillsRequired -and
    [string]$wound[0].XP_TYPE -ceq [string]$e.woundThree.xpType -and
    [string]$wound[0].XP_COST -ceq [string]$e.woundThree.xpCost -and
    [string]$wound[0].XP_CAP -ceq [string]$e.woundThree.xpCap -and
    [string]$wound[0].COMMANDS -ceq [string]$e.woundThree.commands -and
    [string]$wound[0].SKILL_MODS -ceq
        [string]$e.woundThree.skillMods -and
    [string]$wound[0].SCHEMATICS_GRANTED -ceq
        [string]$e.woundThree.schematics -and
    [string]$wound[0].SEARCHABLE -ceq
        [string]$e.woundThree.searchable
) "p14.dancer-wound-three.skill.authentic"

Assert-Contract (
    $fixture.Contains("prepareDancerWoundThree") -and
    $fixture.Contains("purchaseDancerWoundThree") -and
    $fixture.Contains("observeSurrenderDancerWoundThree") -and
    $fixture.Contains("ENTERTAINER_HEALING_XP")
) "p14.dancer-wound-three.fixture.production-and-reversible"
...
if ($Expectation -ceq "Ready")
{
    ...
    Assert-Contract (
        ...
        [int]$b.clientNativeBuild.protocolVersion -eq 55
    ) "p14.dancer-wound-three.build.ready-and-hashed"
    ...
    ) "p14.dancer-wound-three.live-purchase-surrender"
}
if ($failures.Count)
{
    throw "Dancer Wound III contract failed: " + ($failures -join ", ")
}
Write-Host ""
Write-Host "Publish 14.1 Dancer Wound III contract passed."
