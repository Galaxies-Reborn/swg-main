...
$fixtureScript = "test.precu_boost_morale_command_fixture"
...
try
{
    ...
    {
        throw "Boost Morale preflight recovery failed: $recovery"
    }

    $prepared = [string](Invoke-Fixture -Action prepare -Lifecycle $lifecycle)
    $armed = $true
    if ((Get-Field $prepared "grouped") -cne "false" -or
        (Get-Field $prepared "boostMoraleSkill") -cne "true" -or
        (Get-Field $prepared "boostMoraleCommand") -cne "true" -or
    ...
    {
        throw "Boost Morale fixture preparation is not authoritative: $prepared"
    }
    ...
    Invoke-ClientAction -Action QueueBoostMorale -ProcessId $LeaderClientProcessId
    ...
    if ((Get-Field $commandProof "outcome") -cne "passed" -or
        ...
        [int](Get-Field $commandProof "totalWounds") -ne 91 -or
        [int](Get-Field $commandProof "woundsPerMember") -ne 46 -or
        [int](Get-Field $commandProof "woundsPerAttribute") -ne 6 -or
        [int](Get-Field $commandProof "totalWoundsApplied") -ne 91 -or
        [int](Get-Field $commandProof "leaderWounds") -ne 46 -or
        [int](Get-Field $commandProof "memberWounds") -ne 45 -or
    ...
    {
        throw "Boost Morale production command proof failed: $commandProof"
    }
    ...
    {
        throw "Boost Morale cleanup failed: $cleanup"
    }
    $armed = $false
    ...
    {
        throw "Boost Morale idempotent cleanup failed: $idempotent"
    }

    Write-Host "Publish 14.1 boostmorale authenticated two-client runtime passed."
    [pscustomobject]@{
        lifecycle = $lifecycle
...
finally
{
    if ($armed)
    {
        ...
        {
            Write-Warning "Best-effort boostmorale cleanup failed: $_"
        }
    }
}
