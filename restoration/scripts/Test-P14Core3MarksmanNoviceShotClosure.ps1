...
$contract = Get-Content -LiteralPath (Join-Path $restorationRoot ([string]$manifest.contracts.p14Core3MarksmanNoviceShotClosure)) -Raw | ConvertFrom-Json
...
foreach ($path in $paths) { Assert (Test-Path -LiteralPath $path -PathType Leaf) "Missing M257 source: $path" }
...
$expectations = @(
    @{
        name="overChargeShot1"; damage="2.75"; range="64"; animation="fire_1_special_single";
        weapon="RIFLE"; health="0.5"; action="0.5"; mind="0.5"; speed="1.25";
        accuracy="15"; spam="overchargeshot"
    },
    @{
        name="pointBlankSingle1"; damage="2"; range="12"; animation="fire_3_single";
        weapon="PISTOL"; health="0.5"; action="1.25"; mind="0.5"; speed="1.5";
        accuracy="15"; spam="pointblankshot"
    }
)
foreach ($expected in $expectations) {
    ...
    Assert ($d.Count -eq 1 -and $d[0].percentAddFromWeapon -ceq [string]$expected.damage -and
        $d[0].animDefault -ceq [string]$expected.animation -and
        $d[0].attackType -ceq "SINGLE_TARGET" -and
        $d[0].maxRange -ceq [string]$expected.range -and
        $d[0].weaponType -ceq [string]$expected.weapon -and
        $d[0].specialLine -ceq "marksman") "Combat row drifted: $name"
    $o = @($overrides | Where-Object actionName -ceq $name)
    Assert ($o.Count -eq 1 -and
        $o[0].healthCostMultiplier -ceq [string]$expected.health -and
        $o[0].actionCostMultiplier -ceq [string]$expected.action -and
        $o[0].mindCostMultiplier -ceq [string]$expected.mind -and
        $o[0].targetPool -ceq "RANDOM" -and
        $o[0].speedMultiplier -ceq [string]$expected.speed -and
        $o[0].accuracyBonus -ceq [string]$expected.accuracy -and
        $o[0].animationType -ceq "RANGED") "Override drifted: $name"
    $s = @($spam | Where-Object actionName -ceq $name)
    Assert ($s.Count -eq 1 -and
        $s[0].combatSpam -ceq [string]$expected.spam) "Spam row drifted: $name"
}
$owner = @($skills | Where-Object NAME -ceq "combat_marksman_novice")
Assert ($owner.Count -eq 1) "Marksman novice owner row drifted"
foreach ($name in @("overChargeShot1","pointBlankSingle1")) {
    Assert ([string]$owner[0].COMMANDS -match "(^|,)$name(,|$)") "Skill ownership drifted: $name"
}
foreach ($token in @("OVERCHARGE_ONE_COMMAND","POINT_BLANK_SINGLE_ONE_COMMAND",
    "ORIGINAL_OVERCHARGE_ONE_COMMAND","ORIGINAL_POINT_BLANK_SINGLE_ONE_COMMAND",
    "overchargeOneHealthCost=","pointBlankSingleOneHealthCost=",
    "canPerformOverchargeOne=","canPerformPointBlankSingleOne=")) {
...
$patch = Join-Path $restorationRoot "patches/dsrc/255-p14-core3-marksman-novice-shot-closure.patch"
...
if ($Expectation -ceq "Ready") {
    Assert ([string]$contract.status -ceq "ready" -and
        ...
        [string]$contract.runtimeEvidence.result -ceq "passed") "M257 is not Ready"
    foreach ($name in @("overChargeShot1","pointBlankSingle1")) {
...
Write-Host "Publish 14.1 Core3 Marksman novice shot-closure contract passed."
