...
$contract = Get-Content -LiteralPath (Join-Path $restorationRoot ([string]$manifest.contracts.p14Core3BountyHunterDroidResponseBranch)) -Raw | ConvertFrom-Json
...
foreach ($path in $paths.Values) {
    Assert (Test-Path -LiteralPath $path -PathType Leaf) "Missing M326 source: $path"
}
$commands = @("bleedingShot", "eyeShot", "torsoShot")
...
foreach ($name in $commands) {
    ...
    Assert ([string]::IsNullOrEmpty([string]$row.commandCategory) -and
        ...
        [string]$row.validWeapon -ceq "PISTOL" -and
...
$expected = @{
    bleedingShot = @{damage=2.0; speed=1.5; pool="RANDOM"; animation="fire_3_single"; spam="bleedingshot"}
    eyeShot = @{damage=3.0; speed=2.0; pool="MIND"; animation="fire_3_special_single"; spam="eyeshot"}
    torsoShot = @{damage=3.0; speed=2.3; pool="HEALTH"; animation="fire_3_special_single"; spam="torsoshot"}
}
foreach ($name in $commands) {
    ...
    Assert ([double]$combat.percentAddFromWeapon -eq [double]$want.damage -and
        ...
        [string]$combat.anim_pistol -ceq [string]$want.animation -and
    ...
    Assert ([double]$override.healthCostMultiplier -eq 1 -and
        ...
        [string]$override.targetPool -ceq [string]$want.pool -and
...
$bleedingCombat = @($combatRows | Where-Object actionName -CEQ "bleedingShot")[0]
$bleeding = @($overrideRows | Where-Object actionName -CEQ "bleedingShot")[0]
$eye = @($overrideRows | Where-Object actionName -CEQ "eyeShot")[0]
$torsoCombat = @($combatRows | Where-Object actionName -CEQ "torsoShot")[0]
$torso = @($overrideRows | Where-Object actionName -CEQ "torsoShot")[0]
Assert ([string]$bleedingCombat.dotType -ceq "bleeding" -and
    [int]$bleedingCombat.dotIntensity -eq 30 -and
    [int]$bleedingCombat.dotDuration -eq 30 -and
    [string]$bleeding.dotAttribute -ceq "MIND") "bleedingShot DOT drifted"
Assert ([string]$eye.stateEffect1 -ceq "BLIND" -and
    [int]$eye.stateChance1 -eq 100 -and [int]$eye.stateDuration1 -eq 40 -and
    [string]$eye.stateDefense1 -ceq "blind_defense" -and
    [string]$eye.stateJediDefense1 -ceq "jedi_state_defense" -and
    [string]$eye.stateResistance1 -ceq "resistance_states") "eyeShot state drifted"
Assert ([string]$torsoCombat.dotType -ceq "fire" -and
    [int]$torsoCombat.dotIntensity -eq 30 -and
    [int]$torsoCombat.dotDuration -eq 30 -and
    [string]$torso.dotAttribute -ceq "HEALTH") "torsoShot DOT drifted"
...
foreach ($token in @(
    "prepareBountyHunterDroidResponse", "statusBountyHunterDroidResponse",
    "armBountyHunterDroidResponse", "cleanupBountyHunterDroidResponse",
    "BOUNTY_HUNTER_DROID_RESPONSE_SKILLS",
    "BOUNTY_HUNTER_DROID_RESPONSE_COMMANDS",
    "BOUNTY_HUNTER_DROID_RESPONSE_PREREQUISITES",
...
foreach ($key in $hashNames.Keys) {
    Assert ((Sha $paths[$key]) -ceq [string]$contract.buildEvidence.sourceSha256.($hashNames[$key])) "M326 source hash mismatch: $key"
}
if ($Expectation -ceq "Ready") {
    Assert ([string]$contract.status -ceq "ready" -and
        [string]$contract.buildEvidence.result -ceq "passed" -and
        [string]$contract.runtimeEvidence.result -ceq "passed") "M326 is not Ready"
    ...
    Assert ([bool]$runtime.admission.freshDualAuthentication -and
        ...
        [bool]$runtime.admission.clientWeaponStatus.satisfies) "M326 admission proof missing"
    Assert ([bool]$runtime.commands.bleedingShot.serverAccepted -and
        [bool]$runtime.commands.eyeShot.serverAccepted -and
        [bool]$runtime.commands.torsoShot.serverAccepted) "M326 command proof missing"
    Assert ([bool]$runtime.persistence.fullServerRestart -and
        ...
        [bool]$runtime.persistence.postRestartExecution.serverAccepted) "M326 restart proof missing"
    Assert ([bool]$runtime.cleanup.restored -and
        ...
        [int]$runtime.connectionServerCount -eq 1) "M326 cleanup proof missing"
}
Write-Host "Publish 14.1 Core3 Bounty Hunter Droid Response branch contract passed."
