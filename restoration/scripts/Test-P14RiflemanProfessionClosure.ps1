...
$c=Get-Content (Join-Path $root ([string]$m.contracts.p14RiflemanProfessionClosure)) -Raw|ConvertFrom-Json
$skill=Join-Path (Resolve-Path $SourceRoot).Path ([string]$c.sourceFiles.skillTable);$lines=Get-Content $skill;$e=$c.publish14Evidence
...
if($family.Count-ne19-or$compat.Count-ne7-or$hash-cne[string]$e.family.normalizedSortedRowsSha256){throw "Rifleman family check failed"}
if($Expectation-eq"Ready"){$b=$c.buildEvidence;$p=Join-Path $root ([string]$b.overlayPatch-replace"^restoration/","");if((Get-FileHash $skill).Hash.ToLower()-cne[string]$b.sourceSha256."skills.tab"-or(Get-FileHash $p).Hash.ToLower()-cne[string]$b.overlayPatchSha256-or-not$c.runtimeEvidence.graphVisible-or-not$c.runtimeEvidence.serverHealthy){throw "Rifleman ready check failed"}}
Write-Host "Publish 14.1 Rifleman profession closure passed."
