...
$contract =
    Get-Content -LiteralPath (
        Join-Path $restorationRoot (
            [string]$manifest.contracts.p14MedicTier1Progression)
...
foreach ($path in $paths.Values)
{
    ...
    {
        throw "Required materialized Medic tier-I source is missing: $path"
    }
}
...
Write-Host "Publish 14.1 Medic tier-I progression checks:"

$root = $skills.Rows["science_medic"]
Assert-Contract -Condition (
    ...
    -Name "p14.medic-tier1.root.visible-four-by-four"

foreach ($expected in $contract.skills | Select-Object -Skip 1)
{
    ...
    Assert-Contract -Condition (
        ...
        -Name "p14.medic-tier1.skill.$($expected.name)"
}
...
foreach ($group in $contract.schematicGroups.psobject.Properties)
{
    ...
    Assert-Contract -Condition (
        ...
        -Name "p14.medic-tier1.schematic-group.$($group.Name)"
}
...
Assert-Contract -Condition (
    ...
    $fixture.Contains("revokeSkill(player, skillName)") -and
    -not $fixture.Contains("skill.purchaseSkill")) `
    -Name "p14.medic-tier1.fixture.production-operations-no-console-holocron"
...
Assert-Contract -Condition (
    ...
    -Name "p14.medic-tier1.fixture.identity-bound-reversible"
...
if ($Expectation -ceq "Ready")
{
    ...
    Assert-Contract -Condition (
        ...
        [int]$live.clientProtocolVersion -eq 19) `
        -Name "p14.medic-tier1.status.ready"
    Assert-Contract -Condition (
        [bool]$live.novice -and
        ...
        -Name "p14.medic-tier1.live.skills-commands-schematics-modifiers"
    Assert-Contract -Condition (
        [int]$live.medicalXpAfter -eq 0 -and
        [int]$live.craftingXpAfter -eq 0 -and
        [int]$live.availableSkillPointsAfter -eq 227 -and
        [bool]$live.cleanup.restored -and
        [bool]$live.cleanup.idempotent -and
        [bool]$live.serverHealthyAfterCleanup) `
        -Name "p14.medic-tier1.live-costs-cleanup-health"
}

if ($failures.Count -gt 0)
{
    throw "Publish 14.1 Medic tier-I contract failed: $($failures -join ', ')"
}

Write-Host ""
Write-Host "Publish 14.1 Medic tier-I progression contract passed."
