...
if (-not $actions.Contains("public int multiTargetPistolShot(") -or -not $actions.Contains('combatStandardAction("multiTargetPistolShot"')) { throw "Action hook drifted." }
$c = @($commands | Where-Object commandName -ceq $name); if ($c.Count -ne 1 -or $c[0].defaultTime -cne "3.5" -or $c[0].validWeapon -cne "PISTOL" -or $c[0].maxRangeToTarget -cne "64") { throw "Command row drifted." }
$d = @($combat | Where-Object actionName -ceq $name); if ($d.Count -ne 1 -or $d[0].percentAddFromWeapon -cne "4.2" -or $d[0].animDefault -cne "fire_5_special_single" -or $d[0].anim_pistol -cne "fire_5_special_single" -or $d[0].weaponType -cne "PISTOL" -or $d[0].weaponCategory -cne "RANGED_WEAPON" -or $d[0].attackType -cne "AREA" -or $d[0].coneLength -cne "32" -or $d[0].maxRange -cne "64") { throw "Combat row drifted." }
$o = @($overrides | Where-Object actionName -ceq $name); if ($o.Count -ne 1 -or $o[0].healthCostMultiplier -cne "0.5" -or $o[0].actionCostMultiplier -cne "1.25" -or $o[0].mindCostMultiplier -cne "0.5" -or $o[0].targetPool -cne "RANDOM" -or $o[0].speedMultiplier -cne "3.5" -or $o[0].accuracyBonus -cne "50" -or $o[0].animationType -cne "RANGED") { throw "Override row drifted." }
$p = @($spam | Where-Object actionName -ceq $name); if ($p.Count -ne 1 -or $p[0].combatSpam -cne "pistolmultishot") { throw "Spam row drifted." }
...
foreach ($token in @('MARKSMAN_PREPARED', 'combat_pistol_ability_04', 'combat_pistol_speed_04', 'combat_pistol_support_04', 'combat_pistol_master', 'ORIGINAL_BITS', 'hasMultiTargetPistolShot=', 'canMultiTargetPistolShot=')) { if (-not $fixture.Contains($token)) { throw "Fixture drifted: $token" } }
if ($Expectation -eq "Ready") { $contract = Get-Content -LiteralPath (Join-Path $restorationRoot "contracts/p14-core3-multi-target-pistol-shot.json") -Raw | ConvertFrom-Json; if ($contract.status -ne "ready" -or $contract.runtimeEvidence.result -ne "passed" -or $contract.runtimeEvidence.queueRemoval -cne "Success" -or $contract.runtimeEvidence.spamKey -cne "pistolmultishot_hit" -or $contract.runtimeEvidence.configuredTargetPool -ne 3 -or -not $contract.runtimeEvidence.weaponSatisfies -or -not $contract.runtimeEvidence.serverHealthy) { throw "Runtime evidence is not ready." } }
Write-Host "Publish 14.1 Core3 Multi-Target Pistol Shot contract passed."
