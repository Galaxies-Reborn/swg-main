...
$name = "fanShot"
if (-not $actions.Contains("public int fanShot(") -or -not $actions.Contains('combatStandardAction("fanShot"')) { throw "Action hook drifted." }
...
$d = @($combat | Where-Object actionName -ceq $name); if ($d.Count -ne 1 -or $d[0].percentAddFromWeapon -cne "4" -or $d[0].animDefault -cne "fire_area" -or $d[0].anim_pistol -cne "fire_area" -or $d[0].weaponType -cne "PISTOL" -or $d[0].weaponCategory -cne "RANGED_WEAPON" -or $d[0].attackType -cne "CONE" -or $d[0].coneLength -cne "64" -or $d[0].coneWidth -cne "60" -or $d[0].maxRange -cne "64") { throw "Combat row drifted." }
...
$p = @($spam | Where-Object actionName -ceq $name); if ($p.Count -ne 1 -or $p[0].combatSpam -cne "fanshot") { throw "Spam row drifted." }
$k = @($skills | Where-Object name -ceq "combat_pistol_ability_04"); if ($k.Count -ne 1 -or ([string]$k[0].COMMANDS).Split(",") -cnotcontains $name) { throw "Skill owner drifted." }
foreach ($token in @('MARKSMAN_PREPARED', 'combat_pistol_ability_04', 'combat_pistol_speed_04', 'combat_pistol_support_04', 'combat_pistol_master', 'ORIGINAL_BITS', 'hasMultiTargetPistolShot=', 'canMultiTargetPistolShot=', 'hasFanShot=', 'canFanShot=')) { if (-not $fixture.Contains($token)) { throw "Fixture drifted: $token" } }
if ($Expectation -eq "Ready") { $contract = Get-Content -LiteralPath (Join-Path $restorationRoot "contracts/p14-core3-fan-shot.json") -Raw | ConvertFrom-Json; if ($contract.status -ne "ready" -or $contract.runtimeEvidence.result -ne "passed" -or $contract.runtimeEvidence.queueRemoval -cne "Success" -or $contract.runtimeEvidence.spamKey -cne "fanshot_hit" -or $contract.runtimeEvidence.configuredTargetPool -ne 3 -or -not $contract.runtimeEvidence.weaponSatisfies -or -not $contract.runtimeEvidence.serverHealthy) { throw "Runtime evidence is not ready." } }
Write-Host "Publish 14.1 Core3 Fan Shot contract passed."
