...
$c = Get-Content (Join-Path $root ([string]$manifest.contracts.p14SwordsmanProfessionClosure)) -Raw | ConvertFrom-Json
$skill = Join-Path (Resolve-Path $SourceRoot).Path ([string]$c.sourceFiles.skillTable)
$lines = Get-Content $skill
$prefix = [string]$c.publish14Evidence.family.prefix
$excluded = [string]$c.publish14Evidence.excludedCompatibilityPrefix
$family = @($lines | Where-Object { $_.StartsWith($prefix) -and -not $_.StartsWith($excluded) } | Sort-Object)
$text = ($family -join "`n") + "`n"
$sha = [Security.Cryptography.SHA256]::Create()
try { $hash = (($sha.ComputeHash([Text.Encoding]::UTF8.GetBytes($text)) | ForEach-Object { $_.ToString("x2") }) -join "") }
finally { $sha.Dispose() }
$compat = @($lines | Where-Object { $_.StartsWith($excluded) })
if ($family.Count -ne 19 -or $compat.Count -ne 7 -or $hash -cne [string]$c.publish14Evidence.family.normalizedSortedRowsSha256) {
    throw "Swordsman canonical-family check failed."
}
if ($Expectation -ceq "Ready") {
    ...
    ) { throw "Swordsman ready evidence check failed." }
}
Write-Host "Publish 14.1 Swordsman profession closure passed."
