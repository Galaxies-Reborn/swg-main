...
$contract =
    Get-Content -LiteralPath (
        Join-Path $restorationRoot (
            [string]$manifest.contracts.p14MedicTier3Progression)
...
foreach ($path in $paths.Values)
{
    ...
    {
        throw "Required materialized Medic tier-III source is missing: $path"
    }
}
...
Write-Host "Publish 14.1 Medic tier-III progression checks:"
...
foreach ($expected in $contract.skills)
{
    ...
    Assert-Contract -Condition (
        ...
        -Name "p14.medic-tier3.skill.$($expected.name)"
}
...
foreach ($group in $contract.schematicGroups.psobject.Properties)
{
    ...
    Assert-Contract -Condition (
        ...
        -Name "p14.medic-tier3.schematic-group.$($group.Name)"
}
...
Assert-Contract -Condition $templatesPresent `
    -Name "p14.medic-tier3.schematic-templates.retained"

Assert-Contract -Condition (
    $fixture.Contains("PLAYER_OID = 39008597L") -and
    $fixture.Contains("PLAYER_STATION_ID = 1001") -and
    $fixture.Contains("PREPARED_MEDICAL_XP = 10000") -and
    $fixture.Contains("PREPARED_CRAFTING_XP = 3000") -and
    ...
    -Name "p14.medic-tier3.fixture.production-purchase-operations"
...
Assert-Contract -Condition (
    ...
    $fixture.Contains("revokeSkills(player, TIER_THREE)") -and
    $fixture.Contains("revokeSkills(player, PREREQUISITES)") -and
    $fixture.Contains("removeObjVar(player, ROOT)")) `
    -Name "p14.medic-tier3.fixture.identity-bound-reversible"
...
if ($Expectation -ceq "Ready")
{
    ...
    Assert-Contract -Condition (
        ...
        -Name "p14.medic-tier3.status.ready"
    Assert-Contract -Condition (
        [string]$live.prerequisites -ceq "111111111" -and
        ...
        -Name "p14.medic-tier3.live.skills-commands-schematics-modifiers"
    Assert-Contract -Condition (
        [int]$live.medicalXpAfter -eq 0 -and
        [int]$live.medicalXpCapAfter -eq 30000 -and
        [int]$live.craftingXpAfter -eq 0 -and
        [int]$live.craftingXpCapAfter -eq 12000 -and
        [int]$live.availableSkillPointsAfter -eq 199 -and
        [bool]$live.cleanup.restored -and
        [bool]$live.cleanup.idempotent -and
        [bool]$live.serverHealthyAfterCleanup) `
        -Name "p14.medic-tier3.live-costs-cleanup-health"
}

if ($failures.Count -gt 0)
{
    throw "Publish 14.1 Medic tier-III contract failed: $($failures -join ', ')"
}

Write-Host ""
Write-Host "Publish 14.1 Medic tier-III progression contract passed."
