...
$fixtureScript = "test.precu_formup_command_fixture"
...
try
{
    ...
    {
        throw "Formup preflight recovery failed: $recovery"
    }

    $prepared = [string](Invoke-Fixture -Action prepare -Lifecycle $lifecycle)
    $armed = $true
    if ((Get-Field $prepared "grouped") -cne "false" -or
        (Get-Field $prepared "formupSkill") -cne "true" -or
        (Get-Field $prepared "formupCommand") -cne "true" -or
        [int](Get-Field $prepared "leaderDizzy") -ne 0 -or
        [int](Get-Field $prepared "leaderStunned") -ne 1 -or
        [int](Get-Field $prepared "memberDizzy") -ne 1 -or
        [int](Get-Field $prepared "memberStunned") -ne 1)
    {
        throw "Formup fixture preparation is not authoritative: $prepared"
    }
    ...
    $expectedHealthCost = [math]::Max(0, [int][math]::Floor(
        55 - (([int](Get-Field $prepared "leaderStrength") - 300) /
            1200.0) * 55))
    $expectedActionCost = [math]::Max(0, [int][math]::Floor(
        55 - (([int](Get-Field $prepared "leaderQuickness") - 300) /
            1200.0) * 55))
    $expectedMindCost = [math]::Max(0, [int][math]::Floor(
        55 - (([int](Get-Field $prepared "leaderFocus") - 300) /
            1200.0) * 55))
    ...
    Invoke-ClientAction -Action QueueFormup -ProcessId $LeaderClientProcessId
    ...
    if ((Get-Field $commandProof "outcome") -cne "passed" -or
        ...
        [int](Get-Field $commandProof "adjustedBaseCost") -ne 55 -or
        ...
        [int](Get-Field $commandProof "leaderDizzy") -ne 0 -or
        [int](Get-Field $commandProof "leaderStunned") -ne 0 -or
        [int](Get-Field $commandProof "memberDizzy") -ne 0 -or
        [int](Get-Field $commandProof "memberStunned") -ne 0)
    {
        throw "Formup production command proof failed: $commandProof"
    }
    ...
    {
        throw "Formup cleanup failed: $cleanup"
    }
    $armed = $false
    ...
    {
        throw "Formup idempotent cleanup failed: $idempotent"
    }

    Write-Host "Publish 14.1 formup authenticated two-client runtime passed."
    [pscustomobject]@{
        lifecycle = $lifecycle
...
finally
{
    if ($armed)
    {
        ...
        {
            Write-Warning "Best-effort formup cleanup failed: $_"
        }
    }
}
