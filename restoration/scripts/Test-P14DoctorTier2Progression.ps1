...
$contract =
    Get-Content -LiteralPath (
        Join-Path $restorationRoot (
            [string]$manifest.contracts.p14DoctorTier2Progression
        )
    ) -Raw | ConvertFrom-Json
...
foreach ($path in $paths.Values)
{
    ...
    {
        throw "Required materialized Doctor tier-II source is missing: $path"
    }
}
...
Write-Host "Publish 14.1 Doctor tier-II progression checks:"
...
Assert-Contract -Condition (
    ...
    -Name "p14.doctor-tier2.root.visible-four-by-four"
...
foreach ($expected in $contract.skills)
{
    ...
    Assert-Contract -Condition (
        ...
        -Name "p14.doctor-tier2.skill.$($expected.name)"
}

foreach ($expected in $contract.commands)
{
    $row = $commands.Rows[[string]$expected.name]
    Assert-Contract -Condition (
        $null -ne $row -and
        (Get-Column $commands $row "defaultTime") -ceq
            [string]$expected.defaultTime -and
        (Get-Column $commands $row "characterAbility") -ceq
            [string]$expected.characterAbility -and
        (Get-Column $commands $row "tempScript") -ceq
            [string]$expected.tempScript -and
        (Get-Column $commands $row "target") -ceq
            [string]$expected.target -and
        (Get-Column $commands $row "targetType") -ceq
            [string]$expected.targetType -and
        (Get-Column $commands $row "addToCombatQueue") -ceq
            [string]$expected.addToCombatQueue -and
        (Get-Column $commands $row "executeTime") -ceq
            [string]$expected.executeTime) `
        -Name "p14.doctor-tier2.command.$($expected.name)"
}
...
foreach ($group in $contract.schematicGroups.psobject.Properties)
{
    ...
    Assert-Contract -Condition (
        ...
        -Name "p14.doctor-tier2.schematic-group.$($group.Name)"
}
...
Assert-Contract -Condition $templatesPresent `
    -Name "p14.doctor-tier2.schematic-templates.retained"
...
Assert-Contract -Condition (
    ...
    $fixture.Contains("science_doctor_support_01") -and
    ...
    -Name "p14.doctor-tier2.fixture.production-operations"
...
Assert-Contract -Condition (
    ...
    $fixture.Contains('"bactaShot_1"') -and
    $fixture.Contains('"battle_move_mitigate_2"') -and
    $fixture.Contains('"diseaseInnoculation"') -and
    $fixture.Contains('"serotoninInjection"') -and
    $fixture.Contains('"battle_accuracy_mitigate_2"') -and
    $fixture.Contains("removeObjVar(player, ROOT)")) `
    -Name "p14.doctor-tier2.fixture-rejects-nge-and-restores"
...
if ($Expectation -ceq "Ready")
{
    ...
    Assert-Contract -Condition (
        ...
        -Name "p14.doctor-tier2.status.ready"
    Assert-Contract -Condition (
        [string]$live.prerequisites -ceq
            "11111111111111111111111" -and
        [string]$live.tierSkills -ceq "1111" -and
        [string]$live.commands -ceq "1111" -and
        [string]$live.ngeCommands -ceq "000000000" -and
        [string]$live.schematics -ceq "111111111111" -and
        ...
        -Name "p14.doctor-tier2.live-skills-modifiers-schematics"
    Assert-Contract -Condition (
        [int]$live.medicalXpAfter -eq 0 -and
        [int]$live.medicalXpCapAfter -eq 60000 -and
        [int]$live.craftingXpAfter -eq 0 -and
        [int]$live.craftingXpCapAfter -eq 36000 -and
        [int]$live.availableSkillPointsAfter -eq 131 -and
        ...
        -Name "p14.doctor-tier2.live-costs-cleanup-health"
}

if ($failures.Count -gt 0)
{
    throw "Publish 14.1 Doctor tier-II contract failed: $($failures -join ', ')"
}

Write-Host ""
Write-Host "Publish 14.1 Doctor tier-II progression contract passed."
