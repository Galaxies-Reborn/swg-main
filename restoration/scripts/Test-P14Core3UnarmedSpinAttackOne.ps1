...
if (-not $actions.Contains("public int unarmedSpinAttack1(") -or -not $actions.Contains('combatStandardAction("unarmedSpinAttack1"')) { throw "Action hook drifted." }
...
if ($d.Count -ne 1 -or $d[0].percentAddFromWeapon -cne "2" -or
    $d[0].animDefault -cne "combo_3c" -or $d[0].anim_unarmed -cne "combo_3c" -or
    $d[0].weaponType -cne "UNARMED" -or $d[0].weaponCategory -cne "MELEE_WEAPON" -or
    $d[0].attackType -cne "AREA" -or $d[0].coneLength -cne "16" -or $d[0].maxRange -cne "5") { throw "Combat row drifted." }

$o = @($overrides | Where-Object actionName -ceq $name)
if ($o.Count -ne 1 -or $o[0].healthCostMultiplier -cne "1.5" -or
    $o[0].actionCostMultiplier -cne "1.5" -or $o[0].mindCostMultiplier -cne "1.5" -or
    $o[0].targetPool -cne "RANDOM" -or $o[0].speedMultiplier -cne "2" -or
...
if ($p.Count -ne 1 -or $p[0].combatSpam -cne "leapingveermok") { throw "Spam row drifted." }

$k = @($skills | Where-Object name -ceq "combat_brawler_unarmed_04")
if ($k.Count -ne 1 -or ([string]$k[0].COMMANDS).Split(",") -cnotcontains $name) { throw "Skill owner drifted." }

foreach ($token in @('UNARMED_SPIN_ONE_COMMAND = "unarmedSpinAttack1"', 'ORIGINAL_UNARMED_SPIN_ONE_COMMAND', 'unarmedSpinOneCommand=', 'canPerformUnarmedSpinOne=')) {
    if (-not $fixture.Contains($token)) { throw "Fixture drifted: $token" }
}

if ($Expectation -eq "Ready") {
    $contract = Get-Content -LiteralPath (Join-Path $restorationRoot "contracts/p14-core3-unarmed-spin-attack-one.json") -Raw | ConvertFrom-Json
    if ($contract.status -ne "ready" -or $contract.runtimeEvidence.result -ne "passed" -or
        ...
        $contract.runtimeEvidence.spamKey -cne "leapingveermok_hit" -or
...
Write-Host "Publish 14.1 Core3 Unarmed Spin Attack I contract passed."
