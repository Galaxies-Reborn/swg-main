...
$contract =
    Get-Content -LiteralPath (
        Join-Path $restorationRoot (
            [string]$manifest.contracts.p14CureDiseaseCommand
        )
    ) -Raw | ConvertFrom-Json
...
foreach ($path in $paths.Values)
{
    ...
    {
        throw "Required materialized Cure Disease source is missing: $path"
    }
}
...
$row = Get-TableRow -Path $paths.commandTable -Key "cureDisease"

Write-Host "Publish 14.1 Cure Disease command checks:"
Assert-Contract -Condition (
    ...
    [string]$contract.semanticReference.specialization -ceq
        "MMOCoreORB/src/server/zone/objects/creature/commands/CureDiseaseCommand.h" -and
    ...
    -Name "p14.cure-disease.core3.pinned-command"

Assert-Contract -Condition (
    $row.Matches.Count -eq 1 -and
    $row.Header.Count -eq 94 -and
    $row.Values.Count -eq 94 -and
    $row.Values[0] -ceq "cureDisease" -and
    $row.Values[1] -ceq "combat" -and
    $row.Values[3] -ceq "cureDisease" -and
    $row.Values[7] -ceq "5" -and
    $row.Values[8] -ceq "cureDisease" -and
    $row.Values[72] -ceq "player.cmd.cure_disease" -and
    ...
    $row.Values[88] -ceq "5") `
    -Name "p14.cure-disease.table.authentic-optional-nonqueued"
...
Assert-Contract -Condition (
    ...
    -Name "p14.cure-disease.runtime.organic-seven-meter-help-gates"

Assert-Contract -Condition (
    $handler.Contains("dot.isDiseased(target)") -and
    $handler.Contains("healing.findCureDotMedicine") -and
    $handler.Contains("healing.isCureDiseaseMedicine") -and
    $handler.Contains("healing.performCureDisease") -and
    ...
    -Name "p14.cure-disease.runtime.dot-medicine-and-shared-recovery"

Assert-Contract -Condition (
    $healing.Contains("VAR_CURE_DISEASE_COST = 100") -and
    $healing.Contains("heal_type.equals(HEAL_TYPE_MEDICAL_CURE_DISEASE)") -and
    ...
    -Name "p14.cure-disease.runtime.cost-power-range-and-fixed-xp"
...
Assert-Contract -Condition (
    ...
    $fixture.Contains("science_doctor_wound_speed_04") -and
    $fixture.Contains("DISEASE_STRENGTH = 90") -and
    ...
    -Name "p14.cure-disease.live.identity-bound-reversible-fixture"
...
if ($Expectation -ceq "Ready")
{
    ...
    Assert-Contract -Condition (
        ...
        [int]$live.clientProtocolVersion -eq 28) `
        -Name "p14.cure-disease.status.ready"
    Assert-Contract -Condition (
        [int]$live.handlerCalls -eq 1 -and
        [long]$live.targetOid -eq 39008597 -and
        [int]$live.diseaseBefore -eq 90 -and
        [int]$live.diseaseAfter -eq 0 -and
        [int]$live.diseaseReduction -eq 90 -and
        ...
        -Name "p14.cure-disease.live.self-treatment"
    Assert-Contract -Condition (
        ...
        -Name "p14.cure-disease.live.queue-cleanup-and-health"
}

if ($failures.Count -gt 0)
{
    throw "Publish 14.1 Cure Disease contract failed: $($failures -join ', ')"
}

Write-Host ""
Write-Host "Publish 14.1 Cure Disease command contract passed."
