...
$fixtureScript = "test.precu_berserk_one_command_fixture"
...
try
{
    ...
    if ((Get-Field $prepared "outcome") -cne "ready" -or
        ...
        (Get-Field $prepared "skillBits") -cne "11" -or
        [int](Get-Field $prepared "berserkModifier") -ne 0 -or
        [int](Get-Field $prepared "health") -lt 500 -or
        [int](Get-Field $prepared "action") -lt 500 -or
        [int](Get-Field $prepared "mind") -lt 500)
    {
        throw "Berserk-one fixture preparation is not authoritative: $prepared"
    }

    & $bridge -Action QueueBerserk1 -ClientProcessId $ClientProcessId |
        Write-Host
    ...
    $activationPassed =
        ...
        [int](Get-Field $activated "randomRoll") -eq 25 -and
        ...
        $remaining -ge 15 -and $remaining -le 20
    if (-not $activationPassed)
    {
        throw "Berserk-one activation proof failed: $activated"
    }
    ...
    {
        throw "Berserk-one idempotent cleanup failed: $idempotent"
    }

    Write-Host "Publish 14.1 berserk1 authenticated runtime passed."
    [pscustomobject]@{
        lifecycle = $lifecycle
...
finally
{
    if ($armed)
    {
        ...
        {
            Write-Warning "Best-effort berserk1 cleanup failed: $_"
        }
    }
}
