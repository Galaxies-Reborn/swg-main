...
$contract = Get-Content (Join-Path $root ([string]$manifest.contracts.p14MarksmanProfessionClosure)) -Raw | ConvertFrom-Json
...
$noviceObject = @($familyObjects | Where-Object NAME -CEQ "combat_marksman_novice")
...
Write-Host "Publish 14.1 Marksman profession closure checks:"
...
) "p14.marksman-profession.complete-four-by-four-shape"
Assert-C (
...
) "p14.marksman-profession.current-family-with-historical-p14-baseline"
$noviceCerts = @()
...
) "p14.marksman-profession.gr-base-novice-override"
Assert-C (
...
) "p14.marksman-profession.gr-override-owner-and-source"
if ($Expectation -ceq "Ready") {
    ...
    ) "p14.marksman-profession.historical-build-ready-and-preserved"
    Assert-C (
    ...
    ) "p14.marksman-profession.client-publications"
    Assert-C (
        ...
        [string]$runtime.selectedProfession -ceq "combat_marksman_novice" -and
    ...
    ) "p14.marksman-profession.runtime-graph"
}
if ($failures.Count) { throw "Marksman profession closure failed: " + ($failures -join ", ") }
Write-Host "Publish 14.1 Marksman profession closure passed."
