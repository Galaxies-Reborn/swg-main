...
$contract = Get-Content -LiteralPath (
    Join-Path $restorationRoot ([string]$manifest.contracts.p14Core3FullAutoAreaOne)
...
foreach ($path in $paths.Values) {
    Assert (Test-Path -LiteralPath $path -PathType Leaf) "Missing M270 source: $path"
}
...
$command = @($commands | Where-Object commandName -ceq "fullAutoArea1")
Assert ($command.Count -eq 1 -and
    $command[0].scriptHook -ceq "fullAutoArea1" -and
    ...
    "fullAutoArea1 command row drifted"
$owners = @($skills | Where-Object {
    [string]$_.COMMANDS -match "(^|,)fullAutoArea1(,|$)"
})
Assert ($owners.Count -eq 1 -and
    $owners[0].NAME -ceq "combat_carbine_support_01") `
    "fullAutoArea1 retained skill owner drifted"
$combat = @($combatRows | Where-Object actionName -ceq "fullAutoArea1")
Assert ($combat.Count -eq 1 -and
    $combat[0].percentAddFromWeapon -ceq "1.5" -and
    ...
    "fullAutoArea1 combat row drifted"
$override = @($overrides | Where-Object actionName -ceq "fullAutoArea1")
Assert ($override.Count -eq 1 -and
    ...
    "fullAutoArea1 override row drifted"
$spam = @($spamRows | Where-Object actionName -ceq "fullAutoArea1")
Assert ($spam.Count -eq 1 -and $spam[0].combatSpam -ceq "areashot") `
    "fullAutoArea1 spam row drifted"
$actionsText = Get-Content -LiteralPath $paths.actions -Raw
$fixtureText = Get-Content -LiteralPath $paths.fixture -Raw
Assert ($actionsText -match 'public int fullAutoArea1\(' -and
    $actionsText -match '"fullAutoArea1", self, target') `
    "fullAutoArea1 production dispatcher is missing"
foreach ($token in @(
    'CARBINE_SUPPORT_ONE', 'FULL_AUTO_AREA_ONE_COMMAND',
    'ORIGINAL_FULL_AUTO_AREA_ONE_COMMAND', 'retryCarbineRestoration',
    'fullAutoAreaOneCommand=', 'canPerformFullAutoAreaOne=',
    'fullAutoAreaOneHealthCost=', 'fullAutoAreaOnePrecuHamCostModel=')) {
    Assert ($fixtureText.Contains($token)) "M270 fixture token missing: $token"
}
$overlay = Join-Path $restorationRoot "patches/dsrc/268-p14-core3-full-auto-area-one.patch"
Assert ([string]$contract.buildEvidence.overlaySha256 -ceq (Sha $overlay)) `
    "M270 overlay hash drifted"
if ($Expectation -ceq "Ready") {
    Assert ([string]$contract.status -ceq "ready") "M270 contract is not ready"
    Assert ([string]$contract.buildEvidence.result -ceq "passed") "M270 build evidence is not passed"
    Assert ([string]$contract.runtimeEvidence.result -ceq "passed") "M270 runtime evidence is not passed"
}
Write-Host "Publish 14.1 Core3 fullAutoArea1 contract passed."
