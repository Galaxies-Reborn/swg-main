...
if (-not $actions.Contains("public int unarmedHit1(") -or
    -not $actions.Contains('combatStandardAction("unarmedHit1"')) {
    throw "Action hook drifted."
}

$c = @($commands | Where-Object commandName -ceq $name)
if ($c.Count -ne 1 -or $c[0].defaultTime -cne "1.5" -or
...
if ($d.Count -ne 1 -or $d[0].percentAddFromWeapon -cne "2" -or
    $d[0].animDefault -cne "combo_2d" -or
    $d[0].anim_unarmed -cne "combo_2d" -or
...
if ($o.Count -ne 1 -or $o[0].healthCostMultiplier -cne "1" -or
    $o[0].actionCostMultiplier -cne "1" -or
    $o[0].mindCostMultiplier -cne "1" -or
    $o[0].targetPool -cne "RANDOM" -or
    $o[0].speedMultiplier -cne "1.5" -or
...
if ($p.Count -ne 1 -or $p[0].combatSpam -cne "steelhands") {
    throw "Spam row drifted."
}

$k = @($skills | Where-Object name -ceq "combat_brawler_unarmed_01")
if ($k.Count -ne 1 -or ([string]$k[0].COMMANDS).Split(",") -cnotcontains $name) {
    throw "Skill owner drifted."
}

foreach ($token in @(
    'UNARMED_HIT_ONE_COMMAND = "unarmedHit1"',
    'ORIGINAL_UNARMED_HIT_ONE_COMMAND',
    'unarmedHitOneCommand=',
    'canPerformUnarmedHitOne='
)) {
...
if ($Expectation -eq "Ready") {
    $contract = Get-Content -LiteralPath (Join-Path $restorationRoot "contracts/p14-core3-unarmed-hit-one.json") -Raw | ConvertFrom-Json
    if ($contract.status -ne "ready" -or
        ...
        $contract.runtimeEvidence.spamKey -cne "steelhands_hit" -or
...
Write-Host "Publish 14.1 Core3 Unarmed Hit I contract passed."
