...
$contract =
    Get-Content -LiteralPath (
        Join-Path $restorationRoot (
            [string]$manifest.contracts.p14MedicTier4Progression)
...
foreach ($path in $paths.Values)
{
    ...
    {
        throw "Required materialized Medic tier-IV source is missing: $path"
    }
}
...
Write-Host "Publish 14.1 Medic tier-IV progression checks:"
...
foreach ($expected in $contract.skills)
{
    ...
    Assert-Contract -Condition (
        ...
        -Name "p14.medic-tier4.skill.$($expected.name)"
}
...
foreach ($group in $contract.schematicGroups.psobject.Properties)
{
    ...
    Assert-Contract -Condition (
        ...
        -Name "p14.medic-tier4.schematic-group.$($group.Name)"
}
...
Assert-Contract -Condition $templatesPresent `
    -Name "p14.medic-tier4.schematic-templates.retained"

Assert-Contract -Condition (
    $fixture.Contains("PLAYER_OID = 39008597L") -and
    $fixture.Contains("PLAYER_STATION_ID = 1001") -and
    $fixture.Contains("PREPARED_MEDICAL_XP = 15000") -and
    $fixture.Contains("PREPARED_CRAFTING_XP = 6000") -and
    ...
    -Name "p14.medic-tier4.fixture.production-purchase-operations"
...
Assert-Contract -Condition (
    ...
    $fixture.Contains("revokeSkills(player, TIER_FOUR)") -and
    $fixture.Contains("revokeSkills(player, PREREQUISITES)") -and
    $fixture.Contains("removeObjVar(player, ROOT)")) `
    -Name "p14.medic-tier4.fixture.identity-bound-reversible"
...
if ($Expectation -ceq "Ready")
{
    ...
    Assert-Contract -Condition (
        ...
        -Name "p14.medic-tier4.status.ready"
    Assert-Contract -Condition (
        [string]$live.prerequisites -ceq "1111111111111" -and
        [string]$live.tierSkills -ceq "1111" -and
        [string]$live.commands -ceq "11111" -and
        [string]$live.schematics -ceq "111111111" -and
        [int]$live.modifierDeltas.healing_injury_treatment -eq 25 -and
        [int]$live.modifierDeltas.healing_injury_speed -eq 25 -and
        [int]$live.modifierDeltas.healing_ability -eq 10 -and
        [int]$live.modifierDeltas.medical_foraging -eq 20 -and
        [int]$live.modifierDeltas.medicine_assembly -eq 10 -and
        [int]$live.modifierDeltas.medicine_experimentation -eq 10) `
        -Name "p14.medic-tier4.live.skills-commands-schematics-modifiers"
    Assert-Contract -Condition (
        ...
        [int]$live.availableSkillPointsAfter -eq 179 -and
        ...
        -Name "p14.medic-tier4.live-costs-cleanup-health"
}

if ($failures.Count -gt 0)
{
    throw "Publish 14.1 Medic tier-IV contract failed: $($failures -join ', ')"
}

Write-Host ""
Write-Host "Publish 14.1 Medic tier-IV progression contract passed."
