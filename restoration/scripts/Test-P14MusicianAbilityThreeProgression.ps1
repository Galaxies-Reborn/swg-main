...
$contract = Get-Content (
    Join-Path $root ([string]$manifest.contracts.
        p14MusicianAbilityThreeProgression)
...
$e = $contract.publish14Evidence.abilityThree
...
Write-Host "Publish 14.1 Musician Ability III checks:"
Assert-C (
    $box.Count -eq 1 -and
...
) "p14.musician-ability-three.skill.authentic"
Assert-C (
    $fixture.Contains("prepareMusicianAbilityThree") -and
    $fixture.Contains("purchaseMusicianAbilityThree") -and
    $fixture.Contains("observeSurrenderMusicianAbilityThree") -and
    $fixture.Contains("VENTRILOQUISM_ABILITY") -and
    $fixture.Contains("NGE_FIREJET_TWO_ABILITY") -and
    $fixture.Contains("BANDFILL_SCHEMATIC")
) "p14.musician-ability-three.fixture-reversible"
...
if ($Expectation -ceq "Ready")
{
    ...
    Assert-C (
        ...
        [int]$b.clientNativeBuild.protocolVersion -eq 69
    ) "p14.musician-ability-three.build.ready-and-hashed"
    Assert-C (
        [bool]$live.purchase.passed -and
        [int]$live.purchase.musicXpBefore -eq 175000 -and
        [int]$live.purchase.pointCost -eq 3 -and
        [bool]$live.purchase.ventriloquismPresent -and
        -not [bool]$live.purchase.fireJetTwoPresent -and
        [bool]$live.purchase.schematicPresent -and
        [string]$live.purchase.modifierDeltasAfter -ceq "15,20" -and
        [bool]$live.surrender.abilityTwoRetained -and
        [bool]$live.surrender.fireJetRetained -and
        [bool]$live.surrender.trazSchematicRetained -and
        [int]$live.surrender.pointsRecovered -eq 3 -and
        [int]$live.surrender.musicXpCapAfter -eq 700000 -and
        [string]$live.surrender.modifierDeltasAfter -ceq "0,0" -and
        [bool]$live.cleanup.restored -and
        [bool]$live.cleanup.idempotent
    ) "p14.musician-ability-three.live-purchase-surrender"
}
if ($failures.Count)
{
    throw "Musician Ability III contract failed: " +
        ($failures -join ", ")
}
Write-Host ""
Write-Host "Publish 14.1 Musician Ability III contract passed."
