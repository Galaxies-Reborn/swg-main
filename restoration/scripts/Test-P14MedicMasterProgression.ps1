...
$contract =
    Get-Content -LiteralPath (
        Join-Path $restorationRoot (
            [string]$manifest.contracts.p14MedicMasterProgression)
...
foreach ($path in $paths.Values)
{
    ...
    {
        throw "Required materialized Master Medic source is missing: $path"
    }
}
...
Write-Host "Publish 14.1 Master Medic progression checks:"
...
Assert-Contract -Condition (
    ...
    -Name "p14.medic-master.skill.authentic-row"
...
foreach ($group in $contract.schematicGroups.psobject.Properties)
{
    ...
    Assert-Contract -Condition (
        ...
        -Name "p14.medic-master.schematic-group.$($group.Name)"
}
...
Assert-Contract -Condition $templatesPresent `
    -Name "p14.medic-master.schematic-templates.retained"
...
Assert-Contract -Condition (
    ...
    -Name "p14.medic-master.fixture.production-purchase-operations"
...
Assert-Contract -Condition (
    ...
    -Name "p14.medic-master.fixture.identity-bound-reversible"
...
if ($Expectation -ceq "Ready")
{
    ...
    Assert-Contract -Condition (
        ...
        -Name "p14.medic-master.status.ready"
    Assert-Contract -Condition (
        [string]$live.prerequisites -ceq "11111111111111111" -and
        [string]$live.master -ceq "1" -and
        [string]$live.command -ceq "1" -and
        [string]$live.schematic -ceq "1" -and
        [int]$live.modifierDeltas.healing_injury_treatment -eq 25 -and
        [int]$live.modifierDeltas.healing_ability -eq 10 -and
        [int]$live.modifierDeltas.medical_foraging -eq 25) `
        -Name "p14.medic-master.live.skill-command-schematic-modifiers"
    Assert-Contract -Condition (
        ...
        [int]$live.availableSkillPointsAfter -eq 173 -and
        ...
        -Name "p14.medic-master.live-costs-cleanup-health"
}

if ($failures.Count -gt 0)
{
    throw "Publish 14.1 Master Medic contract failed: $($failures -join ', ')"
}

Write-Host ""
Write-Host "Publish 14.1 Master Medic progression contract passed."
