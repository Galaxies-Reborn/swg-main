...
$contract = Get-Content (
    Join-Path $root ([string]$manifest.contracts.
        p14MusicianWoundThreeProgression)
...
$e = $contract.publish14Evidence.woundThree
...
Write-Host "Publish 14.1 Musician Wound III checks:"
Assert-C (
    $box.Count -eq 1 -and
...
) "p14.musician-wound-three.skill.authentic"
Assert-C (
    $fixture.Contains("prepareMusicianWoundThree") -and
    $fixture.Contains("purchaseMusicianWoundThree") -and
    $fixture.Contains("observeSurrenderMusicianWoundThree")
) "p14.musician-wound-three.fixture-reversible"
if ($Expectation -ceq "Ready")
{
    ...
    Assert-C (
        ...
        [int]$b.clientNativeBuild.protocolVersion -eq 73
    ) "p14.musician-wound-three.build.ready-and-hashed"
    Assert-C (
        [bool]$live.purchase.passed -and
        [int]$live.purchase.healingXpBefore -eq 100000 -and
        [int]$live.purchase.pointCost -eq 3 -and
        [int]$live.purchase.modifierDeltaAfter -eq 10 -and
        [bool]$live.surrender.woundTwoRetained -and
    ...
    ) "p14.musician-wound-three.live-purchase-surrender"
}
if ($failures.Count)
{
    throw "Musician Wound III contract failed: " + ($failures -join ", ")
}
Write-Host "Publish 14.1 Musician Wound III contract passed."
