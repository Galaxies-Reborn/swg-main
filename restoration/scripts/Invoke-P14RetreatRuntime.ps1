...
$fixtureScript = "test.precu_retreat_command_fixture"
...
function Invoke-Fixture
{
    param(
        ...
        [ValidateSet("recover", "prepare", "observeGrouped", "observeCommand",
            "observeExpired", "observeUngrouped", "status", "cleanup")]
        [string]$Action,
...
try
{
    ...
    {
        throw "Retreat preflight recovery failed: $recovery"
    }

    $prepared = [string](Invoke-Fixture -Action prepare -Lifecycle $lifecycle)
    $armed = $true
    if ((Get-Field $prepared "grouped") -cne "false" -or
        (Get-Field $prepared "retreatSkill") -cne "true" -or
        (Get-Field $prepared "retreatCommand") -cne "true" -or
        [math]::Abs([double](Get-Field $prepared "leaderMovement") - 1.0) -gt
            0.001 -or
        [math]::Abs([double](Get-Field $prepared "leaderAcceleration") - 1.0) -gt
            0.001 -or
        [math]::Abs([double](Get-Field $prepared "memberMovement") - 1.0) -gt
            0.001 -or
        [math]::Abs([double](Get-Field $prepared "memberAcceleration") - 1.0) -gt
            0.001)
    {
        throw "Retreat fixture preparation is not authoritative: $prepared"
    }
    ...
    $expectedActionCost = [math]::Max(0, [int][math]::Floor(
        82 - (([int](Get-Field $prepared "leaderQuickness") - 300) /
            1200.0) * 82))
    $expectedMindCost = [math]::Max(0, [int][math]::Floor(
        82 - (([int](Get-Field $prepared "leaderFocus") - 300) /
            1200.0) * 82))
    ...
    Invoke-ClientAction -Action QueueRetreat -ProcessId $LeaderClientProcessId
    ...
    if ((Get-Field $commandProof "outcome") -cne "passed" -or
        ...
        [int](Get-Field $commandProof "adjustedBaseCost") -ne 82 -or
        [int](Get-Field $commandProof "actionCost") -ne $expectedActionCost -or
        [int](Get-Field $commandProof "mindCost") -ne $expectedMindCost -or
        [int](Get-Field $commandProof "membersApplied") -ne 1 -or
        [int](Get-Field $commandProof "leaderHealth") -ne $leaderHealthBefore -or
        ...
        [int](Get-Field $commandProof "memberMind") -ne $memberMindBefore -or
        [math]::Abs([double](Get-Field $commandProof "leaderMovement") - 1.0) -gt
            0.001 -or
        [math]::Abs([double](Get-Field $commandProof "leaderAcceleration") - 1.0) -gt
            0.001 -or
        [math]::Abs([double](Get-Field $commandProof "memberMovement") - 1.822) -gt
            0.001 -or
        [math]::Abs([double](Get-Field $commandProof "memberAcceleration") - 1.822) -gt
            0.001)
    {
        throw "Retreat production command proof failed: $commandProof"
    }

    $expiryProof = Wait-FixtureField -Action observeExpired `
        -Lifecycle $lifecycle -Field passed -Value true -TimeoutSeconds 45
    if ((Get-Field $expiryProof "passed") -cne "true" -or
        (Get-Field $expiryProof "memberOutcome") -cne "expired" -or
        [math]::Abs([double](Get-Field $expiryProof "memberMovement") - 1.0) -gt
            0.001 -or
        [math]::Abs([double](Get-Field $expiryProof "memberAcceleration") - 1.0) -gt
            0.001)
    {
        throw "Retreat natural-expiry proof failed: $expiryProof"
    }
    ...
    {
        throw "Retreat cleanup failed: $cleanup"
    }
    $armed = $false
    ...
    {
        throw "Retreat idempotent cleanup failed: $idempotent"
    }

    Write-Host "Publish 14.1 retreat authenticated two-client runtime passed."
    [pscustomobject]@{
        lifecycle = $lifecycle
...
finally
{
    if ($armed)
    {
        ...
        {
            Write-Warning "Best-effort retreat cleanup failed: $_"
        }
    }
}
