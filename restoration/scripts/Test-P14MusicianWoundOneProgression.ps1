...
$contract = Get-Content (
    Join-Path $root ([string]$manifest.contracts.
        p14MusicianWoundOneProgression)
...
$e = $contract.publish14Evidence.woundOne
...
Write-Host "Publish 14.1 Musician Wound I checks:"
Assert-C (
    $box.Count -eq 1 -and
...
) "p14.musician-wound-one.skill.authentic"
Assert-C (
    $fixture.Contains("prepareMusicianWoundOne") -and
    $fixture.Contains("purchaseMusicianWoundOne") -and
    $fixture.Contains("observeSurrenderMusicianWoundOne") -and
    $fixture.Contains("ORIGINAL_HEALING_XP") -and
    $fixture.Contains("MUSICIAN_WOUND_ONE_XP_COST")
) "p14.musician-wound-one.fixture-reversible"
...
if ($Expectation -ceq "Ready")
{
    ...
    Assert-C (
        ...
        [int]$b.clientNativeBuild.protocolVersion -eq 71
    ) "p14.musician-wound-one.build.ready-and-hashed"
    Assert-C (
        [bool]$live.purchase.passed -and
        [int]$live.purchase.healingXpBefore -eq 25000 -and
        [int]$live.purchase.pointCost -eq 5 -and
        [int]$live.purchase.musicWoundModifierDeltaAfter -eq 5 -and
        [int]$live.purchase.healingXpCapAfter -eq 200000 -and
        [bool]$live.surrender.musicianNoviceRetained -and
        [bool]$live.surrender.trazCommandRetained -and
        [bool]$live.surrender.klooHornSchematicRetained -and
        ...
        [int]$live.surrender.musicWoundModifierDeltaAfter -eq 0 -and
        [bool]$live.cleanup.restored -and
        [bool]$live.cleanup.idempotent
    ) "p14.musician-wound-one.live-purchase-surrender"
}
if ($failures.Count)
{
    throw "Musician Wound I contract failed: " +
        ($failures -join ", ")
}
Write-Host ""
Write-Host "Publish 14.1 Musician Wound I contract passed."
