...
$contract =
    Get-Content -LiteralPath (
        Join-Path $restorationRoot (
            [string]$manifest.contracts.p14DoctorTier4Progression
        )
    ) -Raw | ConvertFrom-Json
...
foreach ($path in $paths.Values)
{
    ...
    {
        throw "Required materialized Doctor tier-IV source is missing: $path"
    }
}
...
Write-Host "Publish 14.1 Doctor tier-IV progression checks:"
...
foreach ($expected in $contract.skills)
{
    ...
    Assert-Contract -Condition (
        ...
        -Name "p14.doctor-tier4.skill.$($expected.name)"
}
...
foreach ($expected in $contract.commands)
{
    ...
    Assert-Contract -Condition (
        $null -ne $row -and
        $row.Count -eq [int]$expected.expectedColumnCount -and
        (Get-Column $commands $row "scriptHook") -ceq
            [string]$expected.scriptHook -and
        ...
        (Get-Column $commands $row "executeTime") -ceq
            [string]$expected.executeTime -and
        (Get-Column $commands $row "maxRangeToTarget") -ceq
            [string]$expected.maxRangeToTarget) `
        -Name "p14.doctor-tier4.command.$($expected.name)"
}
...
foreach ($group in $contract.schematicGroups.psobject.Properties)
{
    ...
    Assert-Contract -Condition (
        ...
        -Name "p14.doctor-tier4.schematic-group.$($group.Name)"
}
...
Assert-Contract -Condition $templatesPresent `
    -Name "p14.doctor-tier4.schematic-templates.retained"
...
Assert-Contract -Condition (
    ...
    -Name "p14.doctor-tier4.fixture-production-and-rollback"
...
if ($Expectation -ceq "Ready")
{
    ...
    Assert-Contract -Condition (
        ...
        -Name "p14.doctor-tier4.status.ready"
    Assert-Contract -Condition (
        [string]$live.prerequisites -ceq
            "1111111111111111111111111111111" -and
        [string]$live.tierSkills -ceq "1111" -and
        [string]$live.commands -ceq "1111111" -and
        [string]$live.ngeCommands -ceq
            "000000000000000000000000" -and
        [string]$live.schematics -ceq
        ...
        -Name "p14.doctor-tier4.live-skills-modifiers-schematics"
    Assert-Contract -Condition (
        ...
        [int]$live.availableSkillPointsAfter -eq 111 -and
        ...
        -Name "p14.doctor-tier4.live-costs-cleanup-health"
}

if ($failures.Count -gt 0)
{
    throw "Publish 14.1 Doctor tier-IV contract failed: $($failures -join ', ')"
}

Write-Host ""
Write-Host "Publish 14.1 Doctor tier-IV progression contract passed."
