...
$contract = Get-Content (Join-Path $root ([string]$manifest.contracts.p14MusicianKnowledgeTwoProgression)) -Raw | ConvertFrom-Json
...
$e = $contract.publish14Evidence.knowledgeTwo
...
Write-Host "Publish 14.1 Musician Knowledge II checks:"
Assert-C (
    $box.Count -eq 1 -and
...
) "p14.musician-knowledge-two.skill.authentic"
Assert-C (
    $fixture.Contains("prepareMusicianKnowledgeTwo") -and
    $fixture.Contains("purchaseMusicianKnowledgeTwo") -and
    $fixture.Contains("observeSurrenderMusicianKnowledgeTwo") -and
    $fixture.Contains("FUNK_ABILITY")
) "p14.musician-knowledge-two.fixture-grants-and-reversible"
if ($Expectation -ceq "Ready") {
    ...
    Assert-C (
        ...
        [int]$b.clientNativeBuild.protocolVersion -eq 80
    ) "p14.musician-knowledge-two.build.ready-and-hashed"
    Assert-C (
        ...
        [bool]$live.purchase.bandfillCommandGranted -and
        [bool]$live.purchase.funkCommandGranted -and
        [bool]$live.surrender.knowledgeOneRetained -and
        [int]$live.surrender.pointsRecovered -eq 4 -and
        [int]$live.surrender.musicXpCapAfter -eq 500000 -and
        [bool]$live.cleanup.idempotent
    ) "p14.musician-knowledge-two.live-purchase-surrender"
}
if ($failures.Count) { throw "Musician Knowledge II contract failed: " + ($failures -join ", ") }
Write-Host "Publish 14.1 Musician Knowledge II contract passed."
