...
$contract = Get-Content (Join-Path $root ([string]$manifest.contracts.p14MusicianShockFourProgression)) -Raw | ConvertFrom-Json
...
$e = $contract.publish14Evidence.shockFour
...
Write-Host "Publish 14.1 Musician Shock IV checks:"
Assert-C (
    $box.Count -eq 1 -and
...
) "p14.musician-shock-four.skill.authentic"
Assert-C (
    $fixture.Contains("prepareMusicianShockFour") -and
    $fixture.Contains("purchaseMusicianShockFour") -and
    $fixture.Contains("observeSurrenderMusicianShockFour") -and
    $fixture.Contains("NGE_OMNIBOX_ABILITY")
) "p14.musician-shock-four.fixture-negative-and-reversible"
if ($Expectation -ceq "Ready") {
    ...
    Assert-C (
        ...
        [int]$b.clientNativeBuild.protocolVersion -eq 78
    ) "p14.musician-shock-four.build.ready-and-hashed"
    Assert-C (
        [bool]$live.purchase.passed -and
        [int]$live.purchase.healingXpBefore -eq 125000 -and
        [int]$live.purchase.pointCost -eq 2 -and
        [int]$live.purchase.shockModifierDeltaAfter -eq 25 -and
        -not [bool]$live.purchase.omniboxCommandGranted -and
        -not [bool]$live.purchase.omniboxSchematicGranted -and
        [bool]$live.surrender.shockThreeRetained -and
    ...
    ) "p14.musician-shock-four.live-purchase-surrender"
}
if ($failures.Count) { throw "Musician Shock IV contract failed: " + ($failures -join ", ") }
Write-Host "Publish 14.1 Musician Shock IV contract passed."
