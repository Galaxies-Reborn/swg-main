...
$c = Get-Content (Join-Path $root ([string]$manifest.contracts.p14PistoleerProfessionClosure)) -Raw | ConvertFrom-Json
...
if ($family.Count -ne 19 -or $compat.Count -ne 7 -or $hash -cne [string]$e.family.normalizedSortedRowsSha256) {
    throw "Pistoleer canonical-family check failed."
}
if ($Expectation -ceq "Ready") {
    ...
    ) { throw "Pistoleer ready evidence check failed." }
}
Write-Host "Publish 14.1 Pistoleer profession closure passed."
