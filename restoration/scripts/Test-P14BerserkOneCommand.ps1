...
$contractPath = Join-Path $restorationRoot (
    [string]$manifest.contracts.p14BerserkOneCommand)
...
foreach ($entry in $paths.GetEnumerator())
{
    Assert-Contract (Test-Path -LiteralPath $entry.Value -PathType Leaf) `
        "p14.berserk-one.source.$($entry.Key)"
}

$command = Get-Row -Path $paths.commandTable -Key "berserk1"
Assert-Contract ($null -ne $command) "p14.berserk-one.command.unique"
if ($null -ne $command)
{
    Assert-Contract ($command.Header.Count -eq 94 -and
        $command.Values.Count -eq 94) "p14.berserk-one.command.columns-94"
    Assert-Contract ($command.Row.scriptHook -ceq "berserk1" -and
        $command.Row.failScriptHook -ceq "failSpecialAttack" -and
        $command.Row.tempScript -ceq "") "p14.berserk-one.command.hooks"
    Assert-Contract ($command.Row.target -ceq "other" -and
        ...
        $command.Row.visible -ceq "2") "p14.berserk-one.command.dispatch"
    Assert-Contract ($command.Row.'S:berserk' -ceq "0") `
        "p14.berserk-one.command.no-reactivation"
}

$skillRow = Get-Row -Path $paths.skillTable -Key "combat_brawler_novice"
Assert-Contract ($null -ne $skillRow -and
    $skillRow.Row.COMMANDS -match '(^|,)berserk1(,|"?$)') `
    "p14.berserk-one.skill.ownership"
...
$runnerPath = Join-Path $restorationRoot `
    "scripts/Invoke-P14BerserkOneRuntime.ps1"
$runner = Get-Content -LiteralPath $runnerPath -Raw
$patchPath = Join-Path $restorationRoot `
    "patches/dsrc/243-p14-berserk-one-command.patch"

Assert-Contract ($basePlayer.Contains("public int berserk1(") -and
    $basePlayer.Contains("PRECU_BERSERK_ONE_HEALTH_COST = 100") -and
    $basePlayer.Contains("PRECU_BERSERK_ONE_ACTION_COST = 100") -and
    $basePlayer.Contains("PRECU_BERSERK_ONE_MIND_COST = 50") -and
    $basePlayer.Contains("PRECU_BERSERK_DURATION_SECONDS = 20") -and
    $basePlayer.Contains("rand(0, 100)") -and
    $basePlayer.Contains('getSkillStatMod(self, "berserk")')) `
    "p14.berserk-one.production.policy"
Assert-Contract ($basePlayer.Contains("combat.isMeleeWeapon(weapon)") -and
    ...
    "p14.berserk-one.production.transaction-lifecycle"
Assert-Contract ($fixture.Contains("PLAYER_OID = 44003778L") -and
    ...
    $fixture.Contains('"combat_brawler_novice"') -and
    ...
    "p14.berserk-one.fixture.identity-lifecycle"
Assert-Contract ($runner.Contains("QueueBerserk1") -and
    $runner.Contains('"healthCost"') -and
    $runner.Contains('"berserkState"') -and
    $runner.Contains('"expiredAt"')) `
    "p14.berserk-one.runner.real-client-proof"
Assert-Contract (Test-Path -LiteralPath $patchPath -PathType Leaf) `
    "p14.berserk-one.patch.present"

$hashes = $contract.buildEvidence.sourceSha256
Assert-Contract ((Get-Sha256 $paths.commandTable) -ceq
    [string]$hashes.'command_table.tab') "p14.berserk-one.hash.command-table"
Assert-Contract ((Get-Sha256 $paths.skillTable) -ceq
    [string]$hashes.'skills.tab') "p14.berserk-one.hash.skills"
Assert-Contract ((Get-Sha256 $paths.basePlayer) -ceq
    [string]$hashes.'base_player.java') "p14.berserk-one.hash.base-player"
Assert-Contract ((Get-Sha256 $paths.liveFixture) -ceq
    [string]$hashes.'precu_berserk_one_command_fixture.java') `
    "p14.berserk-one.hash.fixture"
Assert-Contract ((Get-Sha256 $patchPath) -ceq
    [string]$contract.buildEvidence.patchSha256) "p14.berserk-one.hash.patch"
Assert-Contract ((Get-Sha256 $runnerPath) -ceq
    [string]$contract.buildEvidence.runtimeRunnerSha256) `
    "p14.berserk-one.hash.runner"
...
if ($Expectation -ceq "Ready")
{
    Assert-Contract ([string]$contract.status -ceq "ready" -and
        ...
        "p14.berserk-one.status.ready"
    Assert-Contract ([string]$contract.buildEvidence.cleanMaterializationFingerprint `
        -cne "pending") "p14.berserk-one.materialization.fingerprint"
}

if ($failures.Count -gt 0)
{
    throw "Publish 14.1 berserk1 contract failed: $($failures -join ', ')"
}

Write-Host ""
Write-Host "Publish 14.1 berserk1 command contract passed."
