...
$contract = Get-Content (Join-Path $root ([string]$manifest.contracts.p14MusicianKnowledgeThreeProgression)) -Raw | ConvertFrom-Json
...
$e = $contract.publish14Evidence.knowledgeThree
...
Write-Host "Publish 14.1 Musician Knowledge III checks:"
Assert-C (
    $box.Count -eq 1 -and
...
) "p14.musician-knowledge-three.skill.authentic"
Assert-C (
    $fixture.Contains("prepareMusicianKnowledgeThree") -and
    $fixture.Contains("purchaseMusicianKnowledgeThree") -and
    $fixture.Contains("observeSurrenderMusicianKnowledgeThree") -and
    $fixture.Contains("WALTZ_ABILITY")
) "p14.musician-knowledge-three.fixture-grants-and-reversible"
if ($Expectation -ceq "Ready") {
    ...
    Assert-C (
        ...
        [int]$b.clientNativeBuild.protocolVersion -eq 81
    ) "p14.musician-knowledge-three.build.ready-and-hashed"
    Assert-C (
        [bool]$live.purchase.passed -and
        [int]$live.purchase.musicXpBefore -eq 175000 -and
        [int]$live.purchase.pointCost -eq 3 -and
        [int]$live.purchase.musicModifierDeltaAfter -eq 10 -and
        [bool]$live.purchase.waltzCommandGranted -and
        [bool]$live.purchase.flutedroopyCommandGranted -and
        [bool]$live.surrender.knowledgeTwoRetained -and
        [int]$live.surrender.pointsRecovered -eq 3 -and
        [int]$live.surrender.musicXpCapAfter -eq 700000 -and
        [bool]$live.cleanup.idempotent
    ) "p14.musician-knowledge-three.live-purchase-surrender"
}
if ($failures.Count) { throw "Musician Knowledge III contract failed: " + ($failures -join ", ") }
Write-Host "Publish 14.1 Musician Knowledge III contract passed."
